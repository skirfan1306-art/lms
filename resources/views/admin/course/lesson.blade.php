                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            ...
                            <h4 class="card-title mb-2">Syllabus Name - {{ $syllabus->name }}</h4>
                            <h4 class="card-title mb-0">All {{ $lesson->count() }} Lesson </h4>
                        </div><!-- end card header -->
                            ...
                            <div class="listjs-table" id="customerList">
                                <div class="row g-4 mb-3">
                                    <div class="col-sm-auto">
                                        <div>
                                            <a href="{{ route('admin.course.syllabus', $course['id']) }}" class="btn btn-dark"><i class="ri-arrow-left-line"></i> Go Back</a>
                                        </div>
                                    </div>
                                    <div class="col-sm-auto">
                                        <div>
                                            <a href="{{ route('admin.syllabus.lesson.add', $syllabus['id']) }}" class="btn btn-success"><i class="ri-add-line align-bottom me-1"></i> Create New Lesson</a>
                                        </div>
                                    </div>
                                    <div class="col-sm">
...
<table class="table align-middle table-nowrap">
    <thead class="table-light">
        <tr>
            ...
            <th data-sort="three">File Type</th>
            <th data-sort="status">Status</th>
            <th>Action</th>
        </tr>
    </thead>
    <tbody class="list form-check-all" id="lessonSortable">
        ...
        <tr class="handle" data-id="{{ $data['id'] }}">
            ...
            <td class="three">
                @if(!empty($data['file_type']) && !empty($data['file_name']))
                    {{ $data['file_type'] . ' ' . pathinfo($data['file_name'], PATHINFO_EXTENSION) }}
                @elseif($data['file_type'] == 'mcq')
                    {{ $data->mcq_count }} MCQ
                @else
                    Not Selected
                @endif
            ...
            <td>
                <div class="d-flex gap-2">
                    ...
                    <a href="{{ route('admin.syllabus.lesson.edit', $data['id']) }}" class="btn btn-sm btn-success"> <i class="bx bx-pen"></i> </a>
                        ...
                        <div class="modal-body">
                            <div class="mt-2 text-center">
                                ...
                                <div class="mt-4 pt-2 fs-15 mx-4 mx-sm-5">
                                    <h4>Are you Sure ?</h4>
                                    <p class="text-muted mx-4 mb-0">Are you Sure You want to Delete this Lesson ?</p>
                                </div>
                            </div>
                            <div class="d-flex gap-2 justify-content-center mt-4 mb-2">
                                <button type="button" class="btn w-sm btn-light" data-bs-dismiss="modal">Close</button>
                                <form action="{{ route('admin.syllabus.lesson.delete') }}" method="POST">
                                    @csrf
...
document.addEventListener('DOMContentLoaded', function () {
    var tbody = document.getElementById('lessonSortable');
    if (!tbody) return;
    ...
    function saveNewOrder(tbody) {
        ...
        fetch("{{ route('admin.syllabus.lesson.sort') }}", {
            method: "POST",
            headers: {
